(function($templateCache){   'use strict';
  ...
  $templateCache.put('scripts/shaders/particle.show.fs',
    "precision mediump float;uniform sampler2D uSampler;uniform float uTimer;float rand(vec2 co){return fract(sin(dot(co,vec2(32.45884,64.23432))*74.582974)*49.9478593);}void main(void){float sinTime=sin(uTimer/100.0);float cosTime=cos(uTimer/100.0);float tanTime=tan(uTimer/100.0);float randomNum=rand(vec2(sinTime,cosTime));float randomNum2=rand(vec2(cosTime,tanTime));float randomNum3=rand(vec2(tanTime,sinTime));vec3 rgb=vec3(randomNum,randomNum2,randomNum3);gl_FragColor=vec4(rgb,1.0);}"
  );


  $templateCache.put('scripts/shaders/particle.show.vs',
    "attribute vec3 aVertexPosition;uniform mat4 uMVMatrix;uniform mat4 uPMatrix;uniform sampler2D uSampler;void main(void){vec2 oPos=aVertexPosition.xy+vec2(0.5/200.0,0.5/200.0);vec3 newPos=texture2D(uSampler,oPos).xyz*200.0-100.0;gl_Position=uPMatrix*uMVMatrix*vec4(newPos,1.0);}"
  );


  $templateCache.put('scripts/shaders/particle.simulate.fs',
    "precision highp float;uniform sampler2D uSampler;varying vec2 vTextureCoord;uniform float uTimer;float rand(vec2 co){return fract(sin(dot(co.xy,vec2(59.454233,72.223432))*0.32457)*2342.0);}void main(){vec4 frameColor=vec4(1.0);vec4 frameColor2=texture2D(uSampler,vTextureCoord);float randomVal=rand(vec2(vTextureCoord.x,vTextureCoord.y));float sinTime=sin(uTimer/250.0);float cosTime=cos(uTimer/250.0);float tanTime=tan(uTimer/250.0);vec4 randomColor=vec4(sinTime*sin(uTimer*vTextureCoord.s),cosTime*cos(uTimer*vTextureCoord.t),sinTime*sin(uTimer*randomVal),1.0);gl_FragColor=randomColor+frameColor2;}"
  );
